...
export function generateClaudeMd(config: ProjectConfig): OutputFile {
  ...
  const lines = [
    ...
    '### Skill Bypass',
    '',
    'When invoking any skill (via `/command`), temporarily lift write restrictions:',
    '```bash',
    'rm -f .harness/sprint-guard-active   # lift restrictions',
    '# ... skill executes freely ...',
    'touch .harness/sprint-guard-active   # restore restrictions',
    '```',
    '',
    '## Security Boundary',
    '',
